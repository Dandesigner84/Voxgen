...
export const verifyCorporateCredentials = (email: string, pass: string): boolean => {
  const corpPass = localStorage.getItem(`corp_user_${email}`);
  if (corpPass && corpPass === pass) return true;
  if (email.endsWith('@voxgen.corporate') && pass === 'corp123') return true;
  return false;
};
